    ...
    if (isset($_POST['maj_profil'])) {
        ...
        if ($nom === '' || $username === '') {
            ...
            if ($st->fetchColumn()) {
                ...
                /* Fiche client liée */
                $pdo->prepare("UPDATE clients SET nom=?, email=?, telephone=?, adresse=?, entreprise=? WHERE id=?")
                    ->execute([$nom, $email, $tel, $adresse, $entreprise, $cid]);
                $_SESSION['admin_nom'] = $nom;
                $ok = 'Profil mis à jour.';
                $u = $pdo->query("SELECT * FROM users WHERE id=$uid")->fetch();
                $CLIENT = $pdo->query("SELECT * FROM clients WHERE id=$cid")->fetch();
            }
        }
    }
...
$aMotDePasse = trim((string)$u['password']) !== '';
client_header('Mon profil', 'profil', $settings, $CLIENT);
?>
<?php if ($bienvenue): ?>
<div class="panel glass" style="border-left:4px solid var(--gold)">
  <h2 style="border:0;margin:0 0 6px">👋 Bienvenue <?= e($CLIENT['nom']) ?> !</h2>
  <p style="color:var(--ink-dim)">Votre compte a été créé via Google. Complétez vos informations pour finaliser votre profil.</p>
</div>
<?php endif; ?>
...
<div class="panel glass">
  ...
  <form method="post">
    ...
    <div class="form-grid">
      <div class="field"><label>Nom complet *</label><input class="input" name="nom" value="<?= e($CLIENT['nom']) ?>" required></div>
      <div class="field"><label>Nom d'utilisateur *</label><input class="input" name="username" value="<?= e($u['username']) ?>" required></div>
      <div class="field"><label>Entreprise</label><input class="input" name="entreprise" value="<?= e($CLIENT['entreprise'] ?? '') ?>"></div>
      <div class="field"><label>Email</label><input class="input" type="email" name="email" value="<?= e($CLIENT['email'] ?? '') ?>"></div>
      <div class="field"><label>Téléphone</label><input class="input" name="telephone" value="<?= e($CLIENT['telephone'] ?? '') ?>"></div>
      <div class="field"><label>Adresse</label><input class="input" name="adresse" value="<?= e($CLIENT['adresse'] ?? '') ?>"></div>
    </div>
