...
$sysDefs = [
    ...
    'proformas' => ['📋','Proformas',    "SELECT f.id, f.numero, f.date_emission AS d, c.nom AS client, c.entreprise, c.type_client FROM factures f JOIN documents_auth a ON a.type='proforma' AND a.doc_id=f.id LEFT JOIN clients c ON c.id=f.client_id WHERE f.type='proforma' ORDER BY f.date_emission DESC, f.id DESC", 'pdf.php?type=proforma&id='],
